...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    def scaled_channels(c):
        return make_divisible(min(c, max_channels) * width, 8)

    def channels_from(index):
        return ch[index]

    ch = [ch]
    layers, save = [], []
    base_modules = (Conv, DWConv, C3k2, SPPF, C2TSSA_DYT_Mona)
    repeat_modules = (C3k2, C2TSSA_DYT_Mona)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):
        ...
        if m in base_modules:
            c1, c2 = channels_from(f), args[0]
            if c2 != nc:
                c2 = scaled_channels(c2)
            args = [c1, c2, *args[1:]]
            ...
            c1, cm, c2 = channels_from(f), args[0], args[1]
            if c2 != nc:
                c2, cm = scaled_channels(c2), scaled_channels(cm)
            args = [c1, cm, c2, *args[2:]]
            if m in (HGBlock, Dynamic_HGBlock):
                args.insert(4, n)
                n = 1
        elif m is nn.BatchNorm2d:
            args = [channels_from(f)]
            c2 = channels_from(f)
        elif m is Concat:
            ...
            c2 = None
        elif m is DynamicAlignFusion:
            c2 = args[0]
            args = [[ch[x] for x in f], c2]
        elif m is MFM:
            c1 = [ch[x] for x in f]
            c2 = scaled_channels(args[0])
            args = [c1, c2, *args[1:]]
        else:
            c2 = channels_from(f)
        ...
        if verbose:
            LOGGER.info(f"{i:>3}{str(f):>20}{n_:>3}{module.np:10.0f}  {module_type:<45}{str(args):<30}")
        save.extend(x % i for x in ([f] if isinstance(f, int) else f) if x != -1)
        layers.append(module)
        if i == 0:
            ch = []
        ch.append(c2 if c2 is not None else ch[f[-1]])
